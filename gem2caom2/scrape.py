...
JSON_FILE_LIST = 'https://archive.gemini.edu/jsonfilelist/notengineering/NotFail/filepre='
JSON_METADATA = 'https://archive.gemini.edu/jsonsummary/canonical/notengineering/NotFail/'


ArchiveJson = namedtuple( 'ArchiveJson', 'json entry_ts_s')

def read_json_file_list_page(start_time_s, last_processed_time_s, session):
    """
    :param start_time_s: Represents a timestamp for the day being queried.
    :param last_processed_time_s: The archive.gemini.edu timestamp for the last file to be processed at CADC.
        Anything newer should be processed.
    :return:
    """
    file_names = {}
    ...
    for telescope_str in ['S', 'N']:
        ...
        logging.error(f'Querying {file_list_url}')
        response = None
        try:
            response = mc.query_endpoint_session(file_list_url, session)
            if response is None:
                logging.warning(f'Could not query {file_list_url}')
            else:
                temp = parse_json_file_list(response.text, last_processed_time_s)
    ...
    for _, value in file_names.items():
        temp[value.entry_ts_s].append(value.json)
    # order the list of work to be done by time
    ordered_file_names = OrderedDict(temp.items())
    return ordered_file_names


def parse_json_file_list(json_string, last_processed_time_s):
    logging.error(f'Begin parse_json_file_list with {last_processed_time_s}')
    work_list = {}
    if not json_string.startswith('[]'):
        temp = Table.read(json_string, format='pandas.json')
        for entry in temp:
            # e.g. 2019-11-01 00:01:34.610517+00:00, and yes, I know about %z
            entry_ts_s = datetime.strptime(entry['lastmod'].replace('+00:00', ''), '%Y-%m-%d %H:%M:%S.%f').timestamp()
            # the same file name can be in the list returned more than once
            if entry_ts_s >= last_processed_time_s:
                f_name = entry['filename']
                if f_name in work_list.keys():
                    for key, value in work_list.items():
                        if f_name == key:
                            existing_ts = value.entry_ts_s
                            break
                    if entry_ts_s > existing_ts:
                        del work_list[f_name]
                        work_list[f_name] = ArchiveJson(entry, entry_ts_s)
                        logging.debug(f'Replacing {entry["filename"]} in work list.')
                else:
                    work_list[f_name] = ArchiveJson(entry, entry_ts_s)
                    logging.debug(f'Adding {f_name} to work list.')
    logging.error(f'End parse_json_file_list with {len(work_list)} entries.')
    return work_list
